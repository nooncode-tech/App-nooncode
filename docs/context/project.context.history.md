...
- Completion status:
  - PARTIAL until migration `0045` is applied to remote and browser validation runs. All code paths, types, tests, and docs are complete and CI-green locally.
  - This closes ADR-010 §Amendment §Implementation hooks #3 (F-V08): the persistence layer does not introduce a client-side read path — the link is surfaced via the existing internal-role-gated proposal read path.
  - Next FASE 1/2 candidates after migration applies (per roadmap §17): (a) merge + apply migration + browser validation to flip this iteration to COMPLETE, (b) F-V07 disclaimer notifications (1h), (c) G8 plan-refs cleanup in `scripts/check-migrations.mjs` (30 min), (d) B1.2 once `whsec_*` is delivered (~15 min) — the remaining critical path to FASE 1 close.
