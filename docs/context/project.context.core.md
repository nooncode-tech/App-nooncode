...
- Admin earnings consolidate endpoint (`POST /api/admin/earnings/consolidate`) is fully implemented backend-side but has no UI surface in `/dashboard/settings`. The Ganancias tab exposes only the credit action; consolidation must be triggered out-of-band (script or direct API) until the UI gap is closed. Functional gap, not a security/data risk. Suggested fix: add a `Consolidar pendiente` panel to the same Ganancias tab reusing the user dropdown + amount field. Documented in `docs/validations/Browser validation 10-05-2026.md`.
