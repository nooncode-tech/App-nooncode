...
## Active risks
- Mock-only surfaces remaining: rewards/points domain and the global users directory still read from in-memory state in `lib/data-context.tsx:442-443`. All other business data (auth, leads, proposals, projects, tasks, wallet, earnings) persists in Supabase. Tracked as R5.3 in the Sprint-5 backlog.
- Local context files can drift quickly unless updated after each real phase.
- Bridge wallet (0025): conversión 1 crédito = $1.00 es temporal. FASE 2 ya implementa acreditación real vía admin, pero no elimina el bridge aún. Tracked as R5.1.
- Stripe live keys pendientes de subir a Vercel antes del primer deploy a producción. Tracked as R1.2.
