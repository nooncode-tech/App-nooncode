...
- Admin earnings consolidate endpoint (`POST /api/admin/earnings/consolidate`) is fully implemented backend-side but has no UI surface in `/dashboard/settings`. The Ganancias tab exposes only the credit action; consolidation must be triggered out-of-band until the UI gap is closed. Functional gap, not a security/data risk. Fix deferred until the v3 earnings state machine is defined; full reasoning, workaround, and reopen criteria in `docs/tdrs/TDR-005-consolidate-earnings-ui-deferred.md`. Detection context in `docs/validations/Browser validation 10-05-2026.md`.
