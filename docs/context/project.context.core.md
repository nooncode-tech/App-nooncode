...
- Distributed rate limiter ships in code as of 2026-05-13 (B14, PR #36): `lib/server/api/rate-limit.ts` now auto-detects `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` and uses `@upstash/ratelimit` sliding window when present, falling back to the previous in-memory Map per-process for dev / local / tests. Fail-open policy logs `rate_limit.upstash.fallback` via `logger.warn` so Upstash outages do not deny all rate-limited traffic. Active risk downgraded from "in-memory will not enforce limits correctly under multi-instance deployment" to: **(a)** until Upstash is provisioned via Vercel Marketplace (operator-side ops), Production still runs on the in-memory branch — same multi-instance correctness gap as before (mitigated by current single-instance pilot traffic); **(b)** Upstash free tier exhaustion would silently shift the limiter into fail-open mode — operator must monitor Upstash dashboard. Detail in `docs/tdrs/TDR-002-rate-limiting-distributed.md`.
