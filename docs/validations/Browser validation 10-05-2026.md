...
- **Action:** **deferred until v3 reshape**. Full debt record + reopen criteria in `docs/tdrs/TDR-005-consolidate-earnings-ui-deferred.md`. Short version: v3 master spec sec. 24.4 introduces a new earnings bucket state machine (`Potential / Confirmed / Pending payout / Paid out / Cancelled`) that does not map onto the current `pending → available_to_withdraw` semantics, and pending FASE 3 automates credit from proposal `paid/won`, so the manual admin credit + consolidate flow becomes a fallback rather than primary path. UI built today would be replaced twice within the v3 reshape.
