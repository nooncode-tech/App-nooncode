...
-- Allow delivery handoff after v0 generation (status = ready) as well as before (pending_generation).
...
begin
  ...
  if workspace_row.current_stage <> 'sales'
    or workspace_row.status not in ('pending_generation', 'ready') then
  ...
  set
    ...
    status = case
      when workspace_row.status = 'ready' then 'delivery_active'::public.prototype_workspace_status
      else workspace_row.status
    end,
