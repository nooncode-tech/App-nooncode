 ...
 *
 * The first six are the original ADR-014 §Orphans reconciliation (2026-05-17).
 * `phase_3r5_outbound_webhook_events_alerted_at` was added 2026-05-30 (ADR-014
 * §Addendum 2026-05-30): it was a follow-up `alerted_at` column applied on
 * 2026-05-26 as its own ledger row, then folded into the single
 * `0062_phase_3r5_outbound_webhook_events.sql` file (the column lives inline at
 * 0062:43). The applied column has no separate disk file, so its ledger row is
 * a genuine orphan — kept here so the drift classifier reports `synced`.
 *
 * @type {ReadonlySet<string>}
 */
export const EXPECTED_ORPHAN_LEDGER_NAMES = new Set([
  ...
  'phase_3r5_outbound_webhook_events_alerted_at',
])
