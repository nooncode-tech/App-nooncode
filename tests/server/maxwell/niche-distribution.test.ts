...
type Entry = { audit: { scoring: { total: number } }; candidate: unknown }
const entry = (score: number): Entry =>
  ({ audit: { scoring: { total: score } }, candidate: {} }) as Entry

function pool(...scores: number[]) {
  return scores.map(entry).sort((a, b) => b.audit.scoring.total - a.audit.scoring.total)
}

test('allocateLeads: no pools → empty', () => {
  // @ts-expect-error - exercising the empty branch defensively
  assert.deepEqual(allocateLeads([]), [])
})

test('allocateLeads: single pool — capped at 5', () => {
  // @ts-expect-error - test fixtures use a structural Entry type
