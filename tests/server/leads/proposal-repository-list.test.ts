...
import { listLeadProposals } from '@/lib/server/leads/proposal-repository'

test('listLeadProposals orders by created_at DESC then id DESC', async () => {
  const client = makeMockClient({ data: [], error: null })

  await listLeadProposals(client as never, 'lead-1', { cursor: null, limit: 10 })
...
test('listLeadProposals requests limit+1 rows from Supabase', async () => {
  const client = makeMockClient({ data: [], error: null })
...
test('listLeadProposals applies composite .or() filter when cursor is non-null', async () => {
  const client = makeMockClient({ data: [], error: null })
  const cursor: CursorPayload = { createdAt: '2026-05-01T10:00:00Z', id: 'abc-123' }

  await listLeadProposals(client as never, 'lead-1', { cursor, limit: 10 })
...
test('listLeadProposals omits .or() when cursor is null', async () => {
  const client = makeMockClient({ data: [], error: null })
...
test('listLeadProposals applies lead_id filter', async () => {
  const client = makeMockClient({ data: [], error: null })

  await listLeadProposals(client as never, 'lead-xyz', { cursor: null, limit: 10 })

  const eqCalls = client._calls.eq
  const leadIdCall = eqCalls.find((c) => c[0] === 'lead_id' && c[1] === 'lead-xyz')
  assert.ok(leadIdCall, 'should call .eq("lead_id", "lead-xyz")')
})
