...
test('listLeadActivities orders by created_at DESC then id DESC', async () => {
  const client = makeMockClient({ data: [], error: null })

  await listLeadActivities(client as never, 'lead-1', { cursor: null, limit: 10 })
...
test('listLeadActivities requests limit+1 rows from Supabase', async () => {
  const client = makeMockClient({ data: [], error: null })
...
test('listLeadActivities applies composite .or() filter when cursor is non-null', async () => {
  const client = makeMockClient({ data: [], error: null })
  const cursor: CursorPayload = { createdAt: '2026-05-01T10:00:00Z', id: 'abc-123' }

  await listLeadActivities(client as never, 'lead-1', { cursor, limit: 10 })
...
test('listLeadActivities omits .or() when cursor is null', async () => {
  const client = makeMockClient({ data: [], error: null })
...
test('listLeadActivities applies lead_id filter', async () => {
  const client = makeMockClient({ data: [], error: null })

  await listLeadActivities(client as never, 'lead-xyz', { cursor: null, limit: 10 })
