...
// ---------------------------------------------------------------------------
// Test 8: Path J — seller fee is capped at activationAmount. Regression
// guard for B1.3a observation §2, 2026-05-17: when a proposal has
// amount=$1 and seller_fee=$100 (data-integrity case from before the
// proposal API guard landed), the seller would otherwise be credited $100
// for a $1 payment. The webhook clamps `sellerFeeAmount` at
// `activationAmount` so the wallet movement can never exceed the payment.
// ---------------------------------------------------------------------------

test('webhook: seller fee cap — $100 selected on $1 activation credits seller only $1', async () => {
  const sellerFee = makeSellerFeeRow(100)
  const confirmedRow = { ...sellerFee, state: 'confirmed', payment_id: PAYMENT_ID }

  const client = makeMockClient({
    maybeSingleByTable: {
      // amount=1 is the bug condition: proposal saved with $1 amount but
      // seller_fee=$100 attached.
      payments: [{ data: { id: PAYMENT_ID, amount: 1, proposal_id: PROPOSAL_ID, project_id: PROJECT_ID }, error: null }],
      lead_proposals: [{ data: { id: PROPOSAL_ID, lead_id: LEAD_ID, amount: 1 }, error: null }],
      leads: [{ data: { lead_origin: 'outbound', assigned_to: SELLER_ID, created_by: SELLER_ID }, error: null }],
      seller_fees: [
        { data: sellerFee, error: null },
        { data: sellerFee, error: null },
    ...
    rpcByName: {
      activate_paid_proposal: [{ data: [activationRpcResult], error: null }],
      credit_wallet_bucket: [
        // Only 1 expected: seller credit. Developer/noon split is 0 because
        // base = max(1 - 1, 0) = 0, so no dev/noon rows get inserted.
        { data: null, error: null },
      ],
    },
  })
  ...
  const sellerRow = earningRows.find((r) => r.actor_role === 'seller')
  assert.equal(sellerRow?.amount, 1, 'seller earning must be capped at activationAmount, not the persisted $100')
  assert.match(
    sellerRow!.notes,
    /capped from \$100/,
    'notes should explain the cap so audit trail is honest',
  )

  // Developer and noon rows must NOT be inserted: base = max(1 - 1, 0) = 0.
  const developerRow = earningRows.find((r) => r.actor_role === 'developer')
  assert.equal(developerRow, undefined, 'no developer row when base = 0 after cap')
  const noonRow = earningRows.find((r) => r.actor_role === 'noon')
  assert.equal(noonRow, undefined, 'no noon row when base = 0 after cap')

  // Wallet credit only fires for the seller (the single row with non-null actor_id).
  const walletCredits = client._rpcCalls.filter((c) => c.name === 'credit_wallet_bucket')
  assert.equal(walletCredits.length, 1, 'only the seller wallet is credited when base = 0')
  const sellerCreditArgs = walletCredits[0].args as { p_amount: number; p_profile_id: string }
  assert.equal(sellerCreditArgs.p_amount, 1, 'seller wallet credit equals the capped amount')
})

test('webhook: seller fee NOT capped when amount >= seller fee (happy path unaffected)', async () => {
  const sellerFee = makeSellerFeeRow(300)
  const confirmedRow = { ...sellerFee, state: 'confirmed', payment_id: PAYMENT_ID }

  const client = makeMockClient({
    maybeSingleByTable: {
      payments: [{ data: { id: PAYMENT_ID, amount: 500, proposal_id: PROPOSAL_ID, project_id: PROJECT_ID }, error: null }],
      lead_proposals: [{ data: { id: PROPOSAL_ID, lead_id: LEAD_ID, amount: 500 }, error: null }],
      leads: [{ data: { lead_origin: 'outbound', assigned_to: SELLER_ID, created_by: SELLER_ID }, error: null }],
      seller_fees: [
        { data: sellerFee, error: null },
        { data: sellerFee, error: null },
  ...
  const sellerRow = earningRows.find((r) => r.actor_role === 'seller')
  assert.equal(sellerRow?.amount, 300, 'seller earning unchanged when no cap is needed')
  assert.doesNotMatch(
    sellerRow!.notes,
    /capped/,
    'notes should not mention capping when amount covers seller fee',
  )

  // base = 500 - 300 = 200; each of dev/noon gets 100.
  const developerRow = earningRows.find((r) => r.actor_role === 'developer')
  assert.equal(developerRow?.amount, 100, 'developer earning = 50% of (500 - 300)')
})
